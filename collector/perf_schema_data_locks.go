...
func (ScrapePerfDataLocks) Scrape(db *sql.DB, ch chan<- prometheus.Metric) error {
	// data_locks
	locksRows, err := db.Query(`SELECT ENGINE, OBJECT_SCHEMA, OBJECT_NAME, LOCK_TYPE, LOCK_MODE, LOCK_STATUS FROM performance_schema.data_locks`)
	if err == nil {
	...
	// data_lock_waits
	waitsRows, err := db.Query(`SELECT REQUESTING_ENGINE_LOCK_ID, BLOCKING_ENGINE_LOCK_ID FROM performance_schema.data_lock_waits`)
	if err == nil {
