...
func TestExporter(t *testing.T) {
	...
	db, err := sql.Open("mysql", FixDSN(dsn))
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()
...
func TestGetMySQLVersion(t *testing.T) {
	...
	convey.Convey("MySQL version extract", t, func() {
		...
		convey.So(getMySQLVersion(db, logger), convey.ShouldEqual, 999)
